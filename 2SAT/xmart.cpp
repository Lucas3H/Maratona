...
int read(){
	...
	fr(i, g.e){
		...
		if(v2 == 0) g.adj[v1 + g.v].insert(v1);
		else if(v1 == 0) g.adj[v2 + g.v].insert(v2);
		else{
			g.adj[v1 + g.v].insert(v2);
			g.adj[v2 + g.v].insert(v1);
		}

		cin >> v1 >> v2;

		if(v1 == 0) g.adj[v2].insert(v2 + g.v);
		else if(v2 == 0) g.adj[v1].insert(v1 + g.v);
		else{
			g.adj[v1].insert(v2 + g.v);
			g.adj[v2].insert(v1 + g.v);
		}
	}
