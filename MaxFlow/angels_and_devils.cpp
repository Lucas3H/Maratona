...
int tipo_lin_block[300][300];
int tipo_col_block[300][300];
...
void build(){
	...
	fr(i, h){
		fr(j, w){
			if(m[i][j] == 'H') addEdge(tipo_lin_block[i][j], tipo_col_block[i][j] + lin_blocks, 1);
		}
	}
