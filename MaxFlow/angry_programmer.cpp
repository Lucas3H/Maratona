// https://onlinejudge.org/index.php?option=onlinejudge&page=show_problem&problem=2501 
#include<bits/stdc++.h>
...
ll Dinic(){
    ...
    while (true){
        for (ll i = 1; i <= 2*v; i++) visted_edges[i] = 0;
        if (!BFS()) break;
...
int read(){
    ...
    E.clear();
	for(int i = 0; i <= 2*v; i++) adj[i].clear();
	idd = 0;
	...
	for(ll i = 0; i < e; i++){
		ll from, to, cap;
		cin >> from >> to >> cap;
		addEdge(from + v, to, cap);
		addEdge(to + v, from, cap);
	}
